...
s=$(kubectl -n consul get pod | grep $name | awk '{print $1}')
if [[ ! -z $s ]] && [[ $s != '' ]]; then 
...
if [[ -z $CONSUL_HTTP_ADDR ]] || [[ $CONSUL_HTTP_ADDR == '' ]]; then
  ...
  node=$(kubectl -n consul get nodes -o=json | jq -r '.items[0].status.addresses[] | select(.type == "ExternalIP") | .address')
  s=$(kubectl -n consul get svc | grep $name | awk '{print $1}')
  if [[ ! -z $s ]] && [[ $s != '' ]]; then 
    echo "OK: Found consul service: $s"
    p=$(kubectl -n consul get svc/$s -o json | jq '.spec.ports[0].nodePort')
